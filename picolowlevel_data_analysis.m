...
for i = 2:length(refval) % Inizia da 2 perché il primo valore non ha precedente
    if isnan(refval(i))
        refval(i) = refval(i-1); % Sostituisci con il valore precedente
    end
end
...
for i = 2:length(rpm) % Inizia da 2 perché il primo valore non ha precedente
    if isnan(rpm(i))
        rpm(i) = rpm(i-1); % Sostituisci con il valore precedente
    end
end
for i = 159:length(tempo) % Inizia da 2 perché il primo valore non ha precedente
    
       time(i-158,1) = tempo(i); % Sostituisci con il valore precedente
       voltage(i-158,1)=0.0117 .* speed2power(i);
       w_rad(i-158,1) = rpm(i) .* (2*3.14/60);         % Converte rpm in rad/s
end
